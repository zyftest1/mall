...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="pc\css\page\account\process.css?1607171728.25">
		<link rel="stylesheet" type="text/css" href="pc\css\page\account\changeMobile.css?1607171728.25">
		...
		<script>
			...
			var Meilishuo = {
				"config": {
					"nt": "1yQyN1tU7ssTVQ5GeZ16w/i6GlxjB9ymmdoeuzbmb38y/+95nvD6B+YHP0C8MT4S",
					"main_site_domain": "/",
					"server_url": "/",
	...
	<body>
		...
		<div class="settings">
			...
			<div class="settings_box"> <strong class="settings_title">手机绑定</strong>
				<div id="process_con"></div>
				<fieldset class="bindMobile">
					<form id="bindMobileForm" method="post" onsubmit="return false" action="">
						<div class="form-list"> <label class="account-label" id="protel" for="mobile">原手机号</label> <input class="lipt l_ipt bgc-f7"
							 autocomplete="off" value="137****1354" disabled="" id="mobile" name="mobile" type="text"> <input class="s_button active_button"
							 type="button" name="s_code" id="s_code" style="" value="发送验证码">
							<a class="lost" href="/account/changeMobile?step=identity">原手机号无法验证?</a>
						</div>
						<!-- 图形验证码，可能需要 -->
						<div class="form-list none" id="acptchaForm">
							<div class="authpic" id="authpic">
								<div class="auth_box">
						...
						<div class="form-list none" id="yanzhengma"> <label for="code" class="account-label">验证码</label> <input class="lipt l_ipt"
							 autocomplete="off" id="code" name="code" type="text" value=""> <strong></strong> </div>
						<div class="sendErrorMsg"></div>
						<div class="submitBox none"> <input id="send_next" name="s_bind" class="ext_submit" type="submit" value="下一步">
							<span class="bindErrorMessage color-pink"></span> </div>
					</form>
					<div class="bie-info">
						<p> 说明： </p>
						<p> 1.修改绑定手机后，原手机号将不能用于登录。您可以使用更换后的手机号进行登录。 </p>
						<p> 2.若因手机号丢失无法进行验证，请联系美丽说客服。 </p>
					</div>
				</fieldset>
			</div>
		</div>
		<script type="text/jsp" id="process">
			<ul class="process">
				<? $.map(this.steps, function(step, index){ ?>
					<li class="<?= step.done?'active':'' ?>" style="width:<?= this.line_length?>px">
						<?= step.msg?>
					</li>
					<? }); ?>
			</ul>
		...
		<script src="pc\~page\pcenter\common\navbar page\account\common\captcha page\account\changeMobile\step1 base.bin?1607171728.25"></script>
		<script>

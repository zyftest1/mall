...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="pc\css\page\account\loginEqm.css?1607171728.25">
		...
		<script>
			...
			var Meilishuo = {
				"config": {
					"nt": "1yQyN1tU7ssTVQ5GeZ16w44vCpKyTGG77Df+187d0M8+0CEt1RP0Qxx0pjZEzxq9",
					"main_site_domain": "/",
					"server_url": "/",
	...
	<body>
		...
		<div class="settings">
			...
			<div class="settings_box"> <strong class="settings_title">登录设备</strong>
				<div class="loginEqm"> </div>
				<div id="neterror" class="neterror"></div>
			</div>
		</div>
		<script type="text/jsp" id="list">
			<? var loginRecord = this.data && this.data.loginRecord || {} ?>
				<table>
					<thead>
						<tr class="tab-con login-title">
							<th>设备类型</th>
							<th>地区</th>
							<th>时间</th>
						</tr>
					...
					<tbody>
						<? loginRecord && loginRecord.map(function(data, index) { ?>
							<tr class="tab-con">
								<td>
									<? if(data.title.indexOf('Mac') >= 0 || data.title.indexOf('Windows') >= 0 || data.title.indexOf('PC') >= 0) { ?> <span class="diannao"><?= data.title ?></span>
										<? } else if (data.title.indexOf('iOS') >= 0) {?> <span class="ios"><?= data.title ?></span>
											<? } else if (data.title.indexOf('Android') >= 0) {?> <span class="android"><?= data.title ?></span>
												<? } ?>
								</td>
								<td>
									<?= data.address ?>
		...
		<script src="pc\~page\pcenter\common\navbar page\account\loginEqm\loginEqm base.bin?1607171728.25"></script>
		<script>
			fml.use('page/pcenter/common/navbar');;
			fml.use('page/account/loginEqm/loginEqm');;
			fml.use('base');;
			fml.iLoad();
		</script>
	</body>
